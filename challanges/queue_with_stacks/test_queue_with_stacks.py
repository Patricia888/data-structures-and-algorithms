...
def test_make_another_queue():
    q = Queue([2, 4, 5, 6, 7])
    assert q.stack_one.top.val == 7
    assert q.stack_one._size == 5
    assert q._size == 5
...
def test_make_queue_and_enqueue():
    q = Queue([2, 4, 5, 6, 7])
    assert q.enqueue(9).stack_one.top.val == 9
    assert q._size == 6
